...
const QUESTS = [
  { id: 'shield', title: 'Forge a Shield', description: 'Craft a wooden shield with a proper grip', section: 'workshop', type: 'build', tier: 'traveller', image: 'shield' },
  { id: 'sword', title: 'Forge a Sword', description: 'Shape a blade and wrap the grip with cord', section: 'workshop', type: 'build', tier: 'traveller', image: 'sword' },
  { id: 'hookboard', title: 'The Hook Board', description: 'Build a board with hooks for your tools', section: 'workshop', type: 'build', tier: 'wanderer', image: 'hookboard' },
  { id: 'joints', title: 'Test of Joints', description: 'Which is strongest—screw, nail, or glue?', section: 'workshop', type: 'experiment', tier: 'wayfarer', image: 'joints' },
  { id: 'safety', title: 'The Rules of Steel', description: 'Learn and record the safety rules of tools', section: 'workshop', type: 'observation', tier: 'wanderer', image: 'safety' },
  { id: 'birdwatch', title: 'Bird Watch', description: 'Find and identify three different birds', section: 'wilds', type: 'observation', tier: 'wanderer', image: 'birdwatch' },
  { id: 'firestart', title: 'Spark to Flame', description: 'Start a fire using natural materials', section: 'wilds', type: 'practice', tier: 'wayfarer', image: 'firestart' },
  { id: 'bread', title: 'The Daily Bread', description: 'Bake a loaf from scratch', section: 'hearth', type: 'build', tier: 'traveller', image: 'bread' },
  { id: 'plant', title: 'Seed to Sprout', description: 'Plant seeds and record their growth', section: 'shire', type: 'project', tier: 'traveller', image: 'plant' },
  { id: 'map', title: 'Chart the Land', description: 'Draw a map of your surroundings', section: 'library', type: 'build', tier: 'wayfarer', image: 'map' },
];
  ...
  return (
    ...
    <div className="app">
      ...
      <nav className="nav-tabs">
        <button className={`nav-tab ${view === 'realms' ? 'active' : ''}`} onClick={() => { setView('realms'); setSelectedRealm(null); }}><span className="nav-tab-icon">🏰</span>Realms</button>
      ...
      <main className="main">
        {view === 'realms' && !selectedRealm && <RealmSelect entries={entries} onSelect={setSelectedRealm} />}
        {view === 'realms' && selectedRealm && <QuestBoard realm={selectedRealm} entries={entries} onBack={() => setSelectedRealm(null)} onSelectQuest={setShowQuestModal} onCustomQuest={() => setShowCustomQuest(true)} />}
      ...
      {showCustomQuest && <CustomQuestModal realm={selectedRealm} onClose={() => setShowCustomQuest(false)} onComplete={addEntry} />}
...
function QuestBoard({ realm, entries, onBack, onSelectQuest, onCustomQuest }) {
  const quests = QUESTS.filter(q => q.section === realm.id).sort((a, b) => TIERS.find(t => t.id === a.tier).rank - TIERS.find(t => t.id === b.tier).rank);
  const completed = entries.filter(e => e.questId).map(e => e.questId);
  return (<><div className="quest-header"><button className="back-btn" onClick={onBack}>←</button><div className="quest-header-info"><h2>{realm.icon} {realm.name}</h2><p>{realm.description}</p></div></div>
...
function QuestModal({ quest, onClose, onComplete }) {
  const [form, setForm] = useState({ title: quest.title, section: quest.section, type: quest.type, tier: quest.tier, questId: quest.id, image: quest.image, responses: ['', '', ''], evidence: [], notes: '' });
...
function CustomQuestModal({ realm, onClose, onComplete }) {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({ title: '', section: realm?.id || 'workshop', type: 'build', tier: 'wanderer', image: 'default', responses: ['', '', ''], evidence: [], notes: '' });
  const fileRef = useRef(null);
  const type = TYPES.find(t => t.id === form.type);
  const tier = TIERS.find(t => t.id === form.tier);
  const handleFile = (e) => Array.from(e.target.files).forEach(f => { const r = new FileReader(); r.onload = ev => setForm(x => ({ ...x, evidence: [...x.evidence, ev.target.result] })); r.readAsDataURL(f); });
...
function Logbook({ entries, onSelectEntry }) {
  ...
  return (<div className="entry-list">{sorted.map((entry, i) => {
    const section = SECTIONS.find(s => s.id === entry.section), tier = TIERS.find(t => t.id === entry.tier);
    return (<div key={entry.id} className="entry-card" onClick={() => onSelectEntry(entry.id)} style={{ animationDelay: `${i * 0.05}s` }}>
      ...
      <div className="entry-content"><div className="entry-header"><div className="entry-title">{entry.title}</div><div className="entry-xp">+{tier?.xp} XP</div></div><div className="entry-meta"><span>{section?.icon} {section?.name}</span><span>{tier?.name}</span><span>{new Date(entry.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span></div></div>
    </div>);
  })}</div>);
}
...
function EntryDetailModal({ entry, onClose, onDelete }) {
  ...
  const section = SECTIONS.find(s => s.id === entry.section), type = TYPES.find(t => t.id === entry.type), tier = TIERS.find(t => t.id === entry.tier);
  return (<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e => e.stopPropagation()}>
    ...
    <div className="modal-body">
      <div className="detail-hero"><div className="detail-image"><QuestImage imageId={entry.image || 'default'} /></div><h2 className="detail-title">{entry.title}</h2><div className="detail-badges"><span className="detail-badge">{tier?.name}</span><span className="detail-badge">+{tier?.xp} XP</span><span className="detail-badge">{section?.name}</span></div></div>
